  ...
  const user = JSON.parse(localStorage.getItem('user'));
    ...
    return (
        <div style={{backgroundColor: "#141d2b"}}>
          {(user === null) && <Nav />}
          {(user !== null) && <Nav2 />}  
            <Box style={boxStyle} padding={10}>
                <Typography variant="h2" className={classes.textVS}>
                    Leaderboard
                </Typography>
                {(user !== null) && (<Typography variant="h6">You are ranked <span style={{color: "#9fef00"}}>#{user.rank}</span></Typography>)}
                
            </Box>
            <Box style={{color: "white"}} padding={5}>
            <Typography className={classes.Leader}>Welcome to the wall of fame - Here are our top 50 users</Typography>
